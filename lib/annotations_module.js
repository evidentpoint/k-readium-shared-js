...
var EpubAnnotationsModule = function (contentDocumentFrame, annotationsManagerView, annotationCSSUrl, spineItem) {
    ...
    EpubAnnotations.HighlightGroup = Backbone.Model.extend({
        ...
        highlightGroupCallback: function (event, type) {
            ...
            var topView = that.get("annotationsManagerView");
            
            if (type === "click") {
                debouncedHighlightCallbackTrigger(topView, "annotationClicked", that.get("type"),
                    that.get("CFI"), that.get("id"), event, documentFrame);
                return;
            }

            if (type === "touchend" || type === "touchmove") {
                debouncedHighlightCallbackTrigger(topView, "annotationTouched", that.get("type"),
                    that.get("CFI"), that.get("id"), event, documentFrame);
                return;
            }

            if (type === "contextmenu") {
                topView.trigger("annotationRightClicked", that.get("type"),
                    that.get("CFI"), that.get("id"), event, documentFrame);
                return;
            }

            if (type === "mousemove") {
                topView.trigger("annotationMouseMove", that.get("type"),
                    that.get("CFI"), that.get("id"), event, documentFrame);
                return;
            }
            ...
            if (type === "mouseenter") {
                topView.trigger("annotationHoverIn", that.get("type"),
                    that.get("CFI"), that.get("id"), event, documentFrame);
            } else if (type === "mouseleave") {
                topView.trigger("annotationHoverOut", that.get("type"),
                    that.get("CFI"), that.get("id"), event, documentFrame);
            }
        ...
        constructHighlightViews: function () {
            ...
            that.set("boundHighlightCallback", function (e) {
                var scale = calculateScale();
            ...
            that.set('boundHighlightElement', $html);
            $html.on(this.getBoundHighlightContainerEvents(), that.get('boundHighlightCallback'));
        },
        ...
        destroyCurrentHighlights: function () {
            ...
            var $el = this.get("boundHighlightElement");
            if ($el) {
                $el.off(events, this.get("boundHighlightCallback"));
            }

            this.set("boundHighlightCallback", null);
            this.set("boundHighlightElement", null);
